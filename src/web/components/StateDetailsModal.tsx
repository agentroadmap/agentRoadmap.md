...
import type { AcceptanceCriterion, Milestone, State } from "../../types";
...
interface Props {
  state?: State; // Optional for create mode
  isOpen: boolean;
  onClose: () => void;
  onSaved?: () => Promise<void> | void; // refresh callback
  onSubmit?: (stateData: Partial<State>) => Promise<void>; // For creating new states
  onArchive?: () => void; // For archiving states
  availableStatuses?: string[]; // Available statuses for new states
...
type StateUpdatePayload = Partial<State> & {
...
type InlineMetaUpdatePayload = Omit<Partial<State>, "milestone"> & {
...
export const StateDetailsModal: React.FC<Props> = ({
  state,
  isOpen,
  onClose,
  onSaved,
  ...
  const [plan, setPlan] = useState(state?.implementationPlan || "");
  const [notes, setNotes] = useState(state?.implementationNotes || "");
  const [finalSummary, setFinalSummary] = useState(state?.finalSummary || "");
  const [criteria, setCriteria] = useState<AcceptanceCriterion[]>(state?.acceptanceCriteriaItems || []);
  const defaultDefinitionOfDone = useMemo(
    () => (definitionOfDoneDefaults ?? []).map((text, index) => ({ index: index + 1, text, checked: false })),
    [definitionOfDoneDefaults],
  );
  const initialDefinitionOfDone = state?.definitionOfDoneItems ?? (isCreateMode ? defaultDefinitionOfDone : []);
  ...
  const [status, setStatus] = useState(state?.status || (isDraftMode ? "Draft" : (availableStatuses?.[0] || "To Do")));
  const [assignee, setAssignee] = useState<string[]>(state?.assignee || []);
  const [labels, setLabels] = useState<string[]>(state?.labels || []);
  const [priority, setPriority] = useState<string>(state?.priority || "");
  const [dependencies, setDependencies] = useState<string[]>(state?.dependencies || []);
  const [references, setReferences] = useState<string[]>(state?.references || []);
  const [milestone, setMilestone] = useState<string>(state?.milestone || "");
  ...
  const baseline = useMemo(() => ({
    ...
    notes: state?.implementationNotes || "",
    finalSummary: state?.finalSummary || "",
    criteria: JSON.stringify(state?.acceptanceCriteriaItems || []),
    definitionOfDone: JSON.stringify(state?.definitionOfDoneItems || (isCreateMode ? defaultDefinitionOfDone : [])),
  }), [state, defaultDefinitionOfDone, isCreateMode]);
  ...
  // Reset local state when state changes or modal opens
  useEffect(() => {
    setTitle(state?.title || "");
    setDescription(state?.description || "");
    setPlan(state?.implementationPlan || "");
    setNotes(state?.implementationNotes || "");
    setFinalSummary(state?.finalSummary || "");
    setCriteria(state?.acceptanceCriteriaItems || []);
    setDefinitionOfDone(state?.definitionOfDoneItems || (isCreateMode ? defaultDefinitionOfDone : []));
    setStatus(state?.status || (isDraftMode ? "Draft" : (availableStatuses?.[0] || "To Do")));
    ...
    setError(null);
    // Preload states for dependency picker
    apiClient.fetchStates().then(setAvailableStates).catch(() => setAvailableStates([]));
  }, [state, isOpen, isCreateMode, isDraftMode, availableStatuses, defaultDefinitionOfDone]);
  ...
  const handleCancelEdit = () => {
    ...
    if (isCreateMode) {
      ...
      setFinalSummary(state?.finalSummary || "");
      setCriteria(state?.acceptanceCriteriaItems || []);
      setDefinitionOfDone(state?.definitionOfDoneItems || []);
      setMode("preview");
    }
  };
  ...
  const buildDefinitionOfDoneCreatePayload = (): StateUpdatePayload => {
  ...
  const buildDefinitionOfDoneEditPayload = (): StateUpdatePayload => {
    const original = state?.definitionOfDoneItems ?? [];
    const cleanedCurrent = normalizeChecklistItems(definitionOfDone);
  ...
  const handleSave = async () => {
    ...
    try {
      const stateData: StateUpdatePayload = {
        title: title.trim(),
        description,
      ...
      if (isCreateMode && onSubmit) {
        Object.assign(stateData, buildDefinitionOfDoneCreatePayload());
        // Create new state
        await onSubmit(stateData);
        // Only close if successful (no error thrown)
        onClose();
      } else if (state) {
        Object.assign(stateData, buildDefinitionOfDoneEditPayload());
        // Update existing state
        await apiClient.updateState(state.id, stateData);
        setMode("preview");
  ...
  const handleToggleCriterion = async (index: number, checked: boolean) => {
    if (!state) return; // Can't toggle in create mode
    if (isFromOtherBranch) return; // Can't toggle for cross-branch states
    // Optimistic update
    const next = (criteria || []).map((c) => (c.index === index ? { ...c, checked } : c));
    setCriteria(next);
    try {
      await apiClient.updateState(state.id, { acceptanceCriteriaItems: next });
      if (onSaved) await onSaved();
    } catch (err) {
      // rollback
  ...
  const handleToggleDefinitionOfDone = async (index: number, checked: boolean) => {
    ...
    try {
      const updates: StateUpdatePayload = checked
        ? { definitionOfDoneCheck: [index] }
        : { definitionOfDoneUncheck: [index] };
      await apiClient.updateState(state.id, updates);
      if (onSaved) await onSaved();
    } catch (err) {
  ...
  const handleInlineMetaUpdate = async (updates: InlineMetaUpdatePayload) => {
    // Don't allow updates for cross-branch states
    ...
    // Only update server if editing existing state
    if (state) {
      try {
        await apiClient.updateState(state.id, updates);
        if (onSaved) await onSaved();
      } catch (err) {
        console.error("Failed to update state metadata", err);
  ...
  // labels handled via ChipInput; no textarea parsing
	...
	const handleComplete = async () => {
		if (!state) return;
		if (!window.confirm("Complete this state? It will be moved to the completed folder.")) return;
		try {
			await apiClient.completeState(state.id);
			if (onSaved) await onSaved();
			onClose();
    } catch (err) {
  ...
  const handleArchive = async () => {
    if (!state || !onArchive) return;
    if (!window.confirm(`Are you sure you want to archive "${state.title}"? This will move the state to the archive folder.`)) return;
    onArchive();
    onClose();
  };
  ...
  return (
    <Modal
      ...
      title={isCreateMode ? (isDraftMode ? "Create New Draft" : "Create New State") : `${displayId} — ${state.title}`}
      ...
      {isFromOtherBranch && (
        <div className="mb-4 flex items-center gap-2 px-4 py-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg text-amber-800 dark:text-amber-200">
          ...
          <div className="flex-1">
            <span className="font-medium">Read-only:</span> This state exists in the <span className="font-semibold">{state?.branch}</span> branch. Switch to that branch to edit it.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              {mode === "preview" ? (
                ...
                <div className="border border-gray-200 dark:border-gray-700 rounded-md">
                  <MDEditor
                    ...
                    data-color-mode={theme}
                    textareaProps={{
                      placeholder: "PR-style summary of what was implemented (write when state is complete)",
                    }}
                  />
                </div>
              )}
        ...
        <div className="md:col-span-1 space-y-4">
          {/* Dates */}
	          {state && (
	            <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3 text-xs text-gray-600 dark:text-gray-300 space-y-1">
	              <div><span className="font-semibold text-gray-800 dark:text-gray-100">Created:</span> <span className="text-gray-700 dark:text-gray-200">{formatStoredUtcDateForDisplay(state.createdDate)}</span></div>
	              {state.updatedDate && (
	                <div><span className="font-semibold text-gray-800 dark:text-gray-100">Updated:</span> <span className="text-gray-700 dark:text-gray-200">{formatStoredUtcDateForDisplay(state.updatedDate)}</span></div>
	              )}
	            </div>
	          )}
          {/* Title (editable for existing states) */}
          {state && (
          ...
          {/* Archive button at bottom of sidebar */}
		          {state && onArchive && !isFromOtherBranch && (
		            <div className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-3">
		              <button
