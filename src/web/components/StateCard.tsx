...
interface StateCardProps {
  state: State;
  onUpdate: (stateId: string, updates: Partial<State>) => void;
...
const StateCard: React.FC<StateCardProps> = ({ state, onEdit, onDragStart, onDragEnd, status, laneId }) => {
  const [isDragging, setIsDragging] = React.useState(false);
  const [showBranchTooltip, setShowBranchTooltip] = React.useState(false);

  // Check if state is from another branch (read-only)
  const isFromOtherBranch = Boolean(state.branch);

  const handleDragStart = (e: React.DragEvent) => {
    // Prevent dragging cross-branch states
  ...
  return (
    <div className="relative">
      {/* Branch tooltip when trying to drag cross-branch state */}
      {showBranchTooltip && isFromOtherBranch && (
        <div className="absolute -top-12 left-1/2 transform -translate-x-1/2 z-50 px-3 py-2 bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-md shadow-lg whitespace-nowrap">
          <div className="flex items-center gap-1">
            ...
            Switch to <span className="font-semibold text-amber-300">{state.branch}</span> branch to move this state
          </div>
      ...
      <div
        ...
        } ${getPriorityClass(state.priority)} ${
      ...
      >
        ...
        {isFromOtherBranch && (
          <div className="flex items-center gap-1.5 mb-2 px-2 py-1 -mx-1 -mt-1 bg-amber-50 dark:bg-amber-900/30 border-b border-amber-200 dark:border-amber-700 rounded-t text-xs text-amber-700 dark:text-amber-300">
            ...
            <span className="truncate">
              From <span className="font-semibold">{state.branch}</span> branch
            </span>
          </div>
        )}

        {/* Header row with priority badge and state ID */}
        <div className="flex items-center justify-between gap-2 mb-1.5">
          <span className="text-xs text-gray-400 dark:text-gray-500 font-mono transition-colors duration-200">{state.id}</span>
          {(() => {
            const badge = getPriorityBadge(state.priority);
            return badge ? (
        ...
        {state.labels.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {state.labels.slice(0, 3).map(label => (
              <span
                key={label}
        ...
        <div className="flex items-center justify-between text-[10px] text-gray-400 dark:text-gray-500 mt-2 pt-1.5 border-t border-gray-100 dark:border-gray-600/50 transition-colors duration-200">
          <span>{formatRelativeDate(state.createdDate)}</span>
          {state.assignee.length > 0 && (
            <span className="truncate max-w-[80px]" title={state.assignee.join(', ')}>
              {state.assignee[0]}
            </span>
          )}
        </div>
