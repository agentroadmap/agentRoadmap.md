...
import type { ReorderStatePayload } from '../lib/api';
...
interface StateColumnProps {
  ...
  onStateReorder?: (payload: ReorderStatePayload) => void;
...
const StateColumn: React.FC<StateColumnProps> = ({
  ...
  const [draggedStateId, setDraggedStateId] = React.useState<string | null>(null);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    const droppedStateId = e.dataTransfer.getData('text/plain');
    ...
    const columnWithoutDropped = states.filter((state) => state.id !== droppedStateId);
    ...
    if (dropPosition) {
      ...
      let count = 0;
      for (let i = 0; i < Math.min(baseIndex, states.length); i += 1) {
        if (states[i]?.id === droppedStateId) {
          continue;
        }
        count += 1;
      }
    ...
    const orderedStateIds = columnWithoutDropped.map((state) => state.id);
    orderedStateIds.splice(insertIndex, 0, droppedStateId);

    const isSameColumn = sourceStatus === title;
    const isOrderUnchanged =
      isSameColumn &&
      orderedStateIds.length === states.length &&
      orderedStateIds.every((stateId, idx) => stateId === states[idx]?.id);
      ...
      <div className="space-y-3">
        {states.map((state, index) => (
          <div 
            ...
            className="relative"
            onDragOver={(e) => {
              if (!onStateReorder || !draggedStateId || draggedStateId === state.id) return;
          ...
          >
            {/* Drop indicator for before this state */}
            ...
            {/* Drop indicator for after this state */}
        ...
        {isDragOver && dragSourceStatus !== title && (
          <div className="border-2 border-green-400 dark:border-green-500 border-dashed rounded-md bg-green-50 dark:bg-green-900/20 p-4 text-center transition-colors duration-200">
            <div className="text-green-600 dark:text-green-400 text-sm font-medium transition-colors duration-200">
              Drop state here to change status
            </div>
          </div>
        )}
        ...
        {onCleanup && title.toLowerCase() === 'done' && states.length > 0 && (
