...
import {
	pickStateForEditWizard,
	runStateCreateWizard,
	runStateEditWizard,
	StateWizardCancelledError,
	type StateWizardPromptRunner,
} from "../commands/state-wizard.ts";
import type { State } from "../types/index.ts";

type PromptResponses = Record<string, string | string[]>;

function createPromptRunner(responses: PromptResponses): StateWizardPromptRunner {
...
describe("state wizard", () => {
	it("builds create input from shared wizard fields", async () => {
		const prompt = createPromptRunner({
			...
			dependencies: "state-1,2",
		});

		const input = await runStateCreateWizard({
		...
		expect(input?.dependencies).toEqual(["STATE-1", "STATE-2"]);
	});
	...
	it("builds prefilled edit update input", async () => {
		...
		const prompt = createPromptRunner({
			...
			documentation: "docs/spec.md",
			dependencies: "state-2,3",
		});

		const updateInput = await runStateEditWizard({
			state: existingState,
			statuses: ["To Do", "In Progress", "Done"],
			promptImpl: prompt,
		});
		...
		expect(updateInput?.dependencies).toEqual(["STATE-2", "STATE-3"]);
	...
	it("supports edit picker flow", async () => {
		...
		const selected = await pickStateForEditWizard({
			states: [
				{ id: "state-3", title: "Third" },
				{ id: "state-2", title: "Second" },
				{ id: "state-1", title: "First" },
			],
			promptImpl: prompt,
		});

		expect(selected).toBe("state-2");
	});

	it("returns null when wizard is cancelled", async () => {
		const cancelledPrompt: StateWizardPromptRunner = async () => {
			throw new StateWizardCancelledError();
		};

		const createInput = await runStateCreateWizard({
			statuses: ["To Do", "Done"],
	...
	it("supports back navigation from step N to N-1 for text prompts", async () => {
		...
		const prompt: StateWizardPromptRunner = async (question) => {
	...
	it("uses select prompts for status and priority with create defaults", async () => {
		...
		const input = await runStateCreateWizard({
			statuses: ["Roadmap", "To Do", "In Progress", "Done"],
			promptImpl: prompt,
		});
		...
		expect(questions.status?.optionValues).toEqual(["Draft", "Roadmap", "To Do", "In Progress", "Done"]);
	...
	it("labels state DoD and single-line text limitations clearly", async () => {
		...
		await runStateCreateWizard({
			statuses: ["To Do", "In Progress", "Done"],
			promptImpl: prompt,
		});

		expect(messages.get("definitionOfDone")).toContain("per-state");
