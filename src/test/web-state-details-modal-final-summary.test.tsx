...
describe("Web state popup Final Summary display", () => {
	...
	it("shows Final Summary editor in create mode", () => {
		...
		const html = renderToString(
			<ThemeProvider>
				<StateDetailsModal isOpen={true} onClose={() => {}} />
			</ThemeProvider>,
		);
	...
	it("resolves numeric milestone aliases to milestone IDs in the milestone selector", () => {
		...
		const state: State = {
			id: "STATE-3",
			title: "State with numeric milestone alias",
			status: "To Do",
			assignee: [],
			createdDate: "2025-01-01",
			labels: [],
		...
		const html = renderToString(
			<ThemeProvider>
				<StateDetailsModal state={state} isOpen={true} onClose={() => {}} milestoneEntities={milestones} />
			</ThemeProvider>,
		);
	...
	it("resolves zero-padded milestone aliases to canonical milestone IDs in the milestone selector", () => {
		...
		const state: State = {
			id: "STATE-4",
			title: "State with zero-padded milestone alias",
			status: "To Do",
			assignee: [],
			createdDate: "2025-01-01",
			labels: [],
	...
	it("prefers archived milestone IDs over active title matches for ID-shaped values", () => {
		...
		const state: State = {
			id: "STATE-5",
			title: "State with ID-shaped collision",
			status: "To Do",
			assignee: [],
			createdDate: "2025-01-01",
			labels: [],
