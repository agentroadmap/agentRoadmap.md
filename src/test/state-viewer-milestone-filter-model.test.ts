...
import { buildStateViewerMilestoneFilterModel } from "../ui/state-viewer-with-search.ts";

describe("state viewer milestone filter model", () => {
	it("builds filter options from active milestones", () => {
		...
		const model = buildStateViewerMilestoneFilterModel(milestones);
