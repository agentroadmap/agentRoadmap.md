 ...
 * 1. Renames any state-*.md files in the drafts folder to draft-*.md
...
/**
 * Migrate draft files from state-*.md to draft-*.md format.
...
export async function migrateDraftPrefixes(fs: FileSystem): Promise<void> {
	...
	// Find all state-*.md files in drafts folder
	let stateFiles: string[];
	try {
		stateFiles = await Array.fromAsync(new Bun.Glob("state-*.md").scan({ cwd: draftsDir, followSymlinks: true }));
	} catch {
	...
	for (const file of stateFiles) {
		...
		try {
			...
			await fs.saveDraft(migratedState);
