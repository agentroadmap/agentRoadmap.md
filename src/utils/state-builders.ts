...
import { normalizeStateId, stateIdsEqual } from "./state-path.ts";

/**
 * Shared utilities for building states and validating dependencies
 * Used by both CLI and MCP to ensure consistent behavior
 */

/**
 * Normalize dependencies to proper state-X format
...
export async function validateDependencies(
	...
	const [states, drafts] = await Promise.all([core.filesystem.listStates(), core.filesystem.listDrafts()]);
	const knownIds = [...states.map((t) => t.id), ...drafts.map((d) => d.id)];
