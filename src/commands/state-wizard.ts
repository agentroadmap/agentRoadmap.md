...
import type { AcceptanceCriterion, State, StateCreateInput, StateUpdateInput } from "../types/index.ts";
import { normalizeDependencies, normalizeStringList } from "../utils/state-builders.ts";

type WizardPriority = "high" | "medium" | "low";

interface StateWizardValues {
	title: string;
	description: string;
	status: string;
	priority: string;
...
export interface StateWizardStateOption {
	id: string;
	title: string;
}
...
interface StateWizardValueQuestion extends Omit<StateWizardQuestion, "name"> {
	name: keyof StateWizardValues;
}

export type StateWizardPromptRunner = (question: StateWizardQuestion) => Promise<Record<string, unknown>>;

export class StateWizardCancelledError extends Error {
	constructor() {
		super("State wizard cancelled.");
	}
}
...
interface WizardOptions {
	statuses: string[];
	promptImpl?: StateWizardPromptRunner;
}
...
const WIZARD_BACKSPACE_NAVIGATION = Symbol("state-wizard-backspace-navigation");
...
const clackPromptRunner: StateWizardPromptRunner = async (question) => {
	if (question.type === "text") {
		...
		if (clack.isCancel(result)) {
			throw new StateWizardCancelledError();
		}
...
async function promptText(
	prompt: StateWizardPromptRunner,
	options: {
		name: keyof StateWizardValues;
		message: string;
		initial?: string;
...
async function runStateWizardValues(params: {
	mode: "create" | "edit";
	statuses: string[];
	initialValues: StateWizardValues;
	promptImpl?: StateWizardPromptRunner;
}): Promise<StateWizardValues | null> {
	...
	try {
		const values: StateWizardValues = {
			...initial,
			status: statusPrompt.initial,
			priority: priorityPrompt.initial,
		};
		const questions: StateWizardValueQuestion[] = [
			{
				type: "text",
				name: "title",
				message: "Title",
			...
			},
			{
				...
				message:
					"State Definition of Done (per-state; project-level DoD configured elsewhere; comma/newline-separated; optional [x]/[ ] prefix per item)",
			},
			{
				type: "text",
			...
			},
			{
				...
				message:
					params.mode === "create"
						? "Dependencies (comma-separated state IDs)"
						: "Dependencies (comma-separated state IDs; blank keeps current value)",
			},
		];
		...
		if (error instanceof StateWizardCancelledError) {
			return null;
		}
		throw error;
	}
...
export async function pickStateForEditWizard(params: {
	states: StateWizardStateOption[];
	promptImpl?: StateWizardPromptRunner;
}): Promise<string | undefined> {
	const prompt = params.promptImpl ?? clackPromptRunner;
	const states = [...params.states].sort((a, b) => a.id.localeCompare(b.id, undefined, { numeric: true }));
	if (states.length === 0) {
		return undefined;
	}
	...
	try {
		const response = await prompt({
			type: "select",
			name: "stateId",
			message: "Select state to edit",
			options: states.map((state) => ({
				label: `${state.id} - ${state.title}`,
				value: state.id,
			})),
		});
		const selected = response.stateId;
...
function toInitialWizardValues(input: { title?: string } & Partial<State>): StateWizardValues {
	return {
		title: input.title ?? "",
...
export async function runStateCreateWizard(
	options: {
		initialTitle?: string;
	} & WizardOptions,
): Promise<StateCreateInput | null> {
	const initialValues = toInitialWizardValues({ title: options.initialTitle ?? "" });
	const values = await runStateWizardValues({
		mode: "create",
...
export async function runStateEditWizard(
	options: {
		state: State;
	} & WizardOptions,
): Promise<StateUpdateInput | null> {
	const initial = toInitialWizardValues(options.state);
	const values = await runStateWizardValues({
		mode: "edit",
		statuses: options.statuses,
	...
	const existingCriteria = (options.state.acceptanceCriteriaItems ?? [])
		.slice()
		.sort((a, b) => a.index - b.index)
	...
	const existingDod = (options.state.definitionOfDoneItems ?? [])
		.slice()
		.sort((a, b) => a.index - b.index)
		.map((entry) => ({ text: entry.text, checked: entry.checked }));
	const targetDod = parseChecklistInput(values.definitionOfDone);
	if (!areChecklistEntriesEqual(existingDod, targetDod)) {
		const existingIndices = (options.state.definitionOfDoneItems ?? []).map((entry) => entry.index);
